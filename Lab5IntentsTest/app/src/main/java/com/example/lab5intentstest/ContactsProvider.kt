...
class ContactsProvider constructor(
 private val context: Context
    ...
    fun getContactsNames(): List<DContact> {
        val contactsList = mutableListOf<DContact>()
...
//            val numberIndex = contactsCursor.getColumnIndex(ContactsContract.Contacts.)
            ...
            while (contactsCursor.moveToNext()) {
                ...
                name?.let { contactsList.add(DContact(id, it)) }
            }
        }
        return contactsList
    }
    ...
    suspend fun getContacts(): List<DContact> = coroutineScope {
...
//            val contactEmails = emails[contact.id].orEmpty()
            DContact(contact.id, contact.name, contactNumbers)
        }
    }
}